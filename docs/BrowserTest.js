...
out = __webpack_require__(/*! ./TestCompatibility */ 1);
...
tryThis = function(out, name, toEval) {
  var error;
  try {
    eval(toEval);
    return out[name + "Ok"] = true;
  } catch (error1) {
    error = error1;
    out.success = false;
    return out[name + "Error"] = error.message;
  }
};
